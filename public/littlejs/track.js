...
function drawScenery()
{
    ...
    for(let i = sceneryDrawDistance; i--; )
    {
        ...
        for(let i=5;i--;)
        {
            ...
            pushTrackSprite(p, vec3(s*m,s,s), hsl(random.float(.33,.4),.6,.2), vec3(1,1));
        }
...
class TrackSegment
{
    constructor(segmentIndex,offset,width)
    {
            ...
            checkpointLine = 1;
        {
            ...
            // Dark grainy road and green grass/ground
            this.colorGround = hsl(.3, .6, .15 + Math.cos(segmentIndex*2/PI)*.02);
            this.colorRoad = hsl(0, 0, stripe ? .2 : .15);
            if (checkpointLine)
