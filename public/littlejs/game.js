...
function gameInit()
{
    ...
    debug && debugInit();
    console.log('debugInit called');
    glInit();
    console.log('glInit called');
    ...
    drawInit();
    console.log('drawInit called');
    inputInit();
    console.log('inputInit called');
    initSounds()
    console.log('initSounds called');
    initGenerative();
    console.log('initGenerative called');
    initHUD();
    console.log('initHUD called');
    buildTrack();
    console.log('buildTrack called');
    gameStart();
...
let forcedPlayerColor = null;
const urlParams = new URLSearchParams(window.location.search);
const vehicleIndexParam = urlParams.get('car');
if (vehicleIndexParam !== null) {
    selectedCarIndex = parseInt(vehicleIndexParam) || 0;
}
const colorParam = urlParams.get('color');
if (colorParam !== null) {
    const result = /^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(colorParam);
    if (result) {
        forcedPlayerColor = new Color(
...
function gameStart()
{
    ...
    let playerColor = forcedPlayerColor;
    if (!playerColor) {
        // Safe hues: Red/Orange/Yellow [0-0.2], Cyan [0.45-0.55], Pink [0.85-1.0]
        const safeHues = [[0.0, 0.2], [0.45, 0.55], [0.85, 1.0]];
        const range = safeHues[Math.floor(Math.random() * safeHues.length)];
        const hue = range[0] + Math.random() * (range[1] - range[0]);
        playerColor = hsl(hue, .8, .5);
    }

    playerVehicle = new PlayerVehicle(2e3, playerColor, selectedCarIndex);
...
function gameUpdateInternal()
{
    ...
    else
    {
        ...
        const maxTraffic = speed > 140 ? 21 : 6;
        const spawnInterval = speed > 140 ? randInt(30, 80) : randInt(100, 250);

        if (vehicles.length < maxTraffic && attractVehicleSpawnTimer-- < 0)
        {
            vehicles.push(new Vehicle(playerVehicle.pos.z-2e3, hsl(rand(),.8,.5)));
            attractVehicleSpawnTimer = spawnInterval;
        }
        ...
        if (false && checkpointTimeLeft > 0 && startCountdown == 0)
        {
